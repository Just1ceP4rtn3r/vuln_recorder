...
项目自带一个示例场景 `scenarios/openplc-delete-user.yaml`：

```bash
# 预览
python -m vuln_recorder run scenarios/openplc-delete-user.yaml --dry-run

# 录制
python -m vuln_recorder run scenarios/openplc-delete-user.yaml --output /tmp/demo
```

测试用场景文件位于 `tests/fixtures/`，涵盖单窗格、双窗格、三窗格、自定义布局等配置。
